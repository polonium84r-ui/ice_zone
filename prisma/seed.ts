/**
 * seed.ts — Idempotent seed for the Thirst. database (PostgreSQL/Prisma).
...
const COUPONS = [
  { code: "WELCOME10", type: "percentage", value: 10, maxDiscount: 50, minOrder: 0, category: null, description: "10% off your first order", active: true },
  { code: "THIRST50", type: "flat", value: 50, maxDiscount: null, minOrder: 299, category: null, description: "₹50 off on orders above ₹299", active: true },
];
...
if (!process.env.THIRST_ADMIN_PASSWORD) {
  console.warn(
    "[seed] THIRST_ADMIN_PASSWORD not set — seeding the admin with a default password. Set it in the environment for production and change it after first login."
  );
}

const USERS = [
  { id: "admin-001", name: "Store Admin", email: ADMIN_EMAIL, password: ADMIN_PASSWORD, phone: "", role: "admin" },
  { id: "staff-001", name: "Counter Staff", email: STAFF_EMAIL, password: STAFF_PASSWORD, phone: "", role: "staff" },
];

async function main() {
  // ---- Menu (versioned full refresh, tracked in a tiny meta table) ----
  await prisma.$executeRawUnsafe(
    `CREATE TABLE IF NOT EXISTS _meta (key TEXT PRIMARY KEY, value TEXT NOT NULL)`
  );
  const rows = await prisma.$queryRawUnsafe<{ value: string }[]>(
    `SELECT value FROM _meta WHERE key = 'menu_version'`
  );
  const currentMenuVersion = rows.length ? Number(rows[0].value) : 0;
  ...
  if (currentMenuVersion < MENU_VERSION) {
    ...
    await prisma.$executeRawUnsafe(
      `INSERT INTO _meta (key, value) VALUES ('menu_version', $1)
  ...
  await prisma.coupon.deleteMany({ where: { code: { in: ["SWEET20", "FLAT50", "KUNAFA10"] } } });
